...
## 简介
[`J = imhmax(I, H)`](#function1)  
[`J = imhmax(I, H, conn)`](#function2)

## 用法
<a id="function1"></a>
[J](#Q4) = imhmax([I](#Q1), [H](#Q2)) 通过 H - 最大值变换抑制灰度图像 `I` 中的区域极大值：该变换会将所有区域极大值的 “高度” 降低最多 `H`，最终完全抑制高度小于 `H` 的区域极大值。

- 区域极大值定义为：由相同灰度值 `t` 构成的连通像素集合，且该集合被灰度值小于 `t` 的像素包围。

<a id="function2"></a>
[J](#Q4) = imhmax([I](#Q1), [H](#Q2), [conn](#Q3)) 额外指定连通性参数 `conn`，用于定义识别区域极大值时的像素连通规则。
...
H - 最大值变换阈值：指定为非负标量，用于控制抑制区域极大值的 “高度阈值”。
