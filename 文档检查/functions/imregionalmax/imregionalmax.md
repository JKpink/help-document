...
## 简介
[`BW = imregionalmax(I)`](#function1)  
[`BW = imregionalmax(I, conn)`](#function2)

## 用法
<a id="function1"></a>
[BW](#Q3) = imregionalmax([I](#Q1)) 返回二值图像 `BW`，其中值为 `1` 的像素标记出灰度图像 `I` 中的区域极大值。区域极大值定义：由像素值恒定的连通像素构成的区域，且该区域的所有外部边界像素值均低于区域内的像素值。
<a id="function2"></a>
[BW](#Q3) = imregionalmax([I](#Q1), [conn](#Q2)) 指定连通性 `conn`。
...
输入的灰度图像：指定为任意维度的数值数组（逻辑数组会被视为数值数组处理）。
...
### 输出参数
**<a id="Q3"></a> BW — 区域最大值的位置**  
逻辑数组

区域极大值的位置标记：返回与 `I` 尺寸相同的逻辑数组，其中：
