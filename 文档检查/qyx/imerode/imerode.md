...
## 简介
[ `J = imerode(I, SE)`](#function1)  
[ `J = imerode(I, nhood)`](#function2)  
[ `J = imerode(___, packopt, m)`](#function3)  
[ `J = imerode(___, shape)`](#function4) 

## 用法
<a id="function1"></a>
[J](#Q7) = imerode([I](#Q1), [SE](#Q2)) 使用结构元素 `SE` 腐蚀灰度、二值或压缩二值图像 `I`。  
<a id="function2"></a>
[J](#Q7)  = imerode([I](#Q1), [nhood](#Q3)) 腐蚀图像 `I`，其中 `nhood` 是由指定结构元素邻域的 0 和 1 组成的矩阵。此语法等效于 imerode(I, strel(nhood))。  
<a id="function3"></a>
[J](#Q7)  = imerode(\_, [packopt](#Q4), [m](#Q5)) 指定输入图像 `I` 是否为压缩二值图像。`m` 指定原始未压缩图像的行维度。  
<a id="function4"></a>
[J](#Q7)  = imerode(\_, [shape](#Q6)) 指定输出图像的大小。
...
腐蚀的图像，以灰度图像、二值图像或压缩二值图像形式返回。如果输入图像 `I` 是压缩二值图像，则 `J` 也是压缩二值图像。`J` 与 `I` 具有相同的数据类型。

## 详细信息
二值腐蚀
用 B 对 A 执行二值腐蚀（记为 A ϴ B）的过程定义为集合运算 A ϴ B = {z|(B<sub>z</sub> ⊆ A}。换句话说，它是满足以下条件的像素位置 z 的集合：平移至位置 z 的结构元素仅与 A 中的前景像素重叠。有关二值腐蚀的详细信息，请参阅 [[1]](#R1)。

灰度腐蚀
在一般形式的 灰度腐蚀中，结构元素有高度。用 A(x, y) 对 B(x, y) 执行灰度腐蚀的过程定义为：
(A ϴ B)(x, y) = min {A (x + x′, y + y′) − B (x′, y′) | (x′, y′) ∊ D<sub>B</sub>}，
DB 是结构元素 B 的域，A(x, y) 假定为图像域外的 +∞。要创建具有非零高度值的结构元素，请使用语法 strel (nhood, height)，其中 height 给出高度值，nhood 对应于结构元素域 D<sub>B</sub>。
灰度腐蚀最常使用平面结构元素 (B (x, y) = 0)。使用这种结构元素的灰度腐蚀等效于使用局部最小值运行符：
(A ϴ B)(x, y) = min {A(x + x′, y + y′) | (x′, y′) ∊ D<sub>B</sub>}。
除了 strel (nhood, height)、strel ('arbitrary', nhood, height) 和 strel ('ball', ...) 之外，所有 strel 语法都生成平面结构元素。

## 提示
- 如果图像 `I` 的维度大于结构元素的维度，则 `imerode` 函数会将相同的形态学腐蚀应用于较高维度上的所有平面。
您可以使用此行为对 RGB 图像执行形态学腐蚀。为 RGB 图像指定二维结构元素，以便对每个颜色通道分别执行运算。
- 当指定结构元素邻域时，`imerode` 通过 floor((size (nhood) + 1) / 2) 确定 [nhood](#Q3) 的中心元素。
- `imerode` 自动利用结构元素对象的分解（如果存在分解）。此外，当使用具有分解的结构元素对象执行二值腐蚀时，`imerode` 会自动使用二值图像压缩来加速腐蚀[[3]](#R1)。
