...
## 简介  
[ `PixelList = bwpixellist(BW)`](#function1)

## 用法  
<a id="function1"></a>
[PixelList](#Q1) = bwpixellist([BW](#Q2))  

对输入的任意维度二值图像 [`BW`](#Q2) ，先按 “n 维全连通规则” 识别所有独立连通区域（2 维为 8 邻域、3 维为 26 邻域、n 维同理）；再按 “从最高维到最低维” 的顺序，提取每个连通区域内所有像素的坐标位置；最终返回多个矩阵，每个矩阵对应一个连通区域，矩阵的每一行都具有 [x y z ...] 形式，指定区域中一个像素的坐标。。
...
### 输出参数  
**<a id="Q1"></a>PixelList — 连通区域的坐标索引集合**  
数值矩阵

返回多个矩阵，每个矩阵对应一个连通区域，矩阵的每一行都具有 [x y z ...] 形式，指定区域中一个像素的坐标。
